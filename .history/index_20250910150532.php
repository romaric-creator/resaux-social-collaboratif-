            ...
            <?php if (!empty($suggested_users)): ?>
                <?php foreach ($suggested_users as $s_user): ?>
                    <div class="suggestion-item">
                        ...
                        <button
                            class="button <?php isFollow($conn, $_SESSION['user_id'], $s_user['id']) > 0 ? "button-secondary " : "button-small follow-btn" ?> "
                            data-user-id="<?php echo $s_user['id']; ?>"><?php
                               if (isFollow($conn, $_SESSION['user_id'], $s_user['id']) > 0) {
                                echo isFollow($conn, $_SESSION['user_id'], $s_user['id'])
                                   echo "Abonné";
                               } else {
                                   echo "Suivre";
                               } ?></button>
