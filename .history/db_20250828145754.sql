-- Table pour les positions sauvegardées par les utilisateurs (lieux d'entreprises)
CREATE TABLE IF NOT EXISTS saved_locations (
  id INT AUTO_INCREMENT PRIMARY KEY,
  user_id INT NOT NULL, -- L'utilisateur qui sauvegarde
  target_user_id INT NOT NULL, -- L'entreprise (user_id) dont la position est sauvegardée
  location VARCHAR(255) NOT NULL,
  date_saved TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
  FOREIGN KEY (target_user_id) REFERENCES users(id) ON DELETE CASCADE,
  UNIQUE KEY unique_saved_location (user_id, target_user_id, location)
);
-- Table pour la gestion des abonnements (followers)
...
-- Pour afficher le nombre d'abonnés et d'abonnements sur un profil :
-- Nombre d'abonnés :
--   SELECT COUNT(*) FROM followers WHERE followed_id = ?;
-- Nombre d'abonnements :
--   SELECT COUNT(*) FROM followers WHERE follower_id = ?;

-- Pour afficher la liste des abonnés :
--   SELECT u.* FROM users u JOIN followers f ON u.id = f.follower_id WHERE f.followed_id = ?;
-- Pour afficher la liste des abonnements :
--   SELECT u.* FROM users u JOIN followers f ON u.id = f.followed_id WHERE f.follower_id = ?;
