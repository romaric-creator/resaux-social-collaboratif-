$(document).ready(function() {
    ...
    // CSS pour le toast (à injecter dynamiquement si absent)
    ...
    $(document).on('click', '#save-location-btn', function() {
        ...
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            $.ajax({
                ...
                error: function() {
                    showToast("Erreur lors de l'envoi de la position.", 'error');
                }
            });
        }, function(error) {
            showToast('Impossible de récupérer la position : ' + error.message, 'error');
        });
    });
$(document).ready(function() {
    ...
    $(document).on('click', '.post-options .options-btn', function(event) {
        ...
        $('.post-options.active').not($(this).closest('.post-options')).removeClass('active');
    ...
    $(document).on('click', '.post-actions .like-btn', function() {
        ...
        $.ajax({
            url: base_url + 'ajax_handler.php', // base_url est défini dans header.php et passé via une variable JS dans chat/index.php
            type: 'POST',
    ...
    $(document).on('click', '.post-actions .comment-btn', function() {
        ...
        const commentsSection = $(this).closest('.post-card').find('.comments-section');
    ...
    $(document).on('submit', '.comments-section .comment-form', function(event) {
        ...
        const commentContent = form.find('input[name="comment_content"]').val().trim();
        const rating = form.find('select[name="rating"]').val();
        const commentList = form.closest('.comments-section').find('.comment-list');
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        const commentsCountSpan = form.closest('.post-card').find('.comments-count');
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (response.data.new_comment) {
                        ...
                        const starsHtml = newComment.note ? displayRatingStarsJS(newComment.note, 'sm') : '';
                        const commentHtml = `
                            <div class="comment-item">
                                <img src="${newComment.user_photo_profil_full_path}" alt="Profil" class="comment-profile-pic">
                                <div class="comment-content">
                                    <span class="comment-author">${newComment.user_nom}</span>
                                    <p>${newComment.contenu.replace(/\n/g, '<br>')}</p>
                                    <span class="comment-time">${newComment.date_commentaire_formatted}</span>
                                    ${starsHtml}
                                </div>
                            </div>
                        `;
                        ...
                        commentsCountSpan.text((parseInt(commentsCountSpan.text()) || 0) + 1 + ' Commentaires');
    ...
    function displayRatingStarsJS(rating, size = '') {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            } else if (i - 0.5 === Math.floor(rating) && rating - Math.floor(rating) >= 0.5) {
    ...
    $(document).on('click', '.company-card-actions .favorite-btn, .profile-page .favorite-btn', function() {
        const button = $(this);
            ...
            success: function(response) {
                if (response.success) {
                    if (response.action === 'followed') {
                        ...
                        button.html('<i class="fas fa-bookmark"></i> Ajouter aux favoris');
                    }
    ...
    $(document).on('submit', '.profile-tab-content .review-form', function(event) {
        ...
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        const reviewsList = form.closest('.profile-tab-content').find('.reviews-list');
    ...
    $(document).on('click', '.report-btn', function() {
        ...
        const confirmReport = confirm("Êtes-vous sûr de vouloir signaler cet utilisateur ?");
        const csrfToken = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        ...
        if (confirmReport) {
            ...
            const raison = prompt("Veuillez indiquer la raison du signalement (obligatoire) :");
