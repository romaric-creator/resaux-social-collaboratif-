...
function getPublications($conn, $filters = [], $sort_by = 'date', $limit = 10, $offset = 0) {
    $sql = "SELECT p.*, u.nom as user_nom, u.photo_profil as user_photo_profil, u.localisation as user_localisation, e.nom as entreprise_nom, e.secteur
            ...
            LEFT JOIN entreprises e ON u.id = e.id_user "; // LEFT JOIN car toutes les publications n'ont pas forcément une entreprise détaillée

    $where_clauses = ["p.status = 'publie'"]; // Seules les publications 'publiees' par défaut
    $params = [];
    ...
    // Filtrage par mots-clés (recherche)
    ...
    // Concaténer toutes les clauses WHERE
    if (!empty($where_clauses)) {
        $sql .= " WHERE " . implode(' AND ', $where_clauses);
    }

    // Pour le tri par popularité, nous devons inclure une jointure supplémentaire et un GROUP BY
    if ($sort_by === 'popularite') {
        $sql .= " LEFT JOIN favoris f ON p.id = f.id_publication ";
        $sql .= " GROUP BY p.id "; // C'est crucial pour COUNT(f.id)
        $sql .= " ORDER BY COUNT(f.id) DESC, p.date_publication DESC";
    } else { // 'date' par défaut ou tout autre tri non spécifié
    ...
    foreach ($params as $key => &$val) {
        // bindValue pour gérer les types int pour LIMIT/OFFSET (sinon, PDO les traite comme string par défaut)
...
function getUsersForSuggestions($conn, $current_user_id, $limit = 5) {
    $stmt = $conn->prepare("SELECT id, nom, photo_profil, type_user, localisation FROM users WHERE id != ? AND type_user != 'admin' ORDER BY RAND() LIMIT ?");
