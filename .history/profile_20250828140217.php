...
<div class="main-content profile-page" style="background: linear-gradient(135deg, #f8fafc 0%, #e9f0fb 100%); min-height: 100vh; padding-bottom: 40px;">
    <div class="profile-header-section">
        <div class="cover-photo">
            ...
            <?php if ($is_current_user_profile): ?>
                <button class="edit-cover-btn button button-small"><i class="fas fa-camera"></i> Modifier la couverture</button>
            <?php endif; ?>
            <div style="position: absolute; left: 50%; bottom: -90px; transform: translateX(-50%); z-index: 20; width: 180px; display: flex; flex-direction: column; align-items: center;">
                <div style="background: #fff; border-radius: 50%; padding: 8px; box-shadow: 0 6px 32px rgba(0,0,0,0.18); border: 5px solid #fff; display: flex; align-items: center; justify-content: center;">
                    <img src="<?php echo getUserProfilePhoto($profile_user['photo_profil']); ?>" alt="Photo de profil" class="profile-pic-large" style="box-shadow: 0 2px 12px rgba(0,0,0,0.18); background: #fff; border: none; width: 160px; height: 160px;">
                </div>
                <?php if ($is_current_user_profile): ?>
                    <button class="edit-profile-pic-btn button button-small" style="margin-top: -24px; position: relative; z-index: 21;"><i class="fas fa-camera"></i> Modifier le profil</button>
                <?php endif; ?>
            </div>
        </div>
        <div class="profile-info-area" style="margin-top: 60px;">
            <h1 style="margin-top: 0.5em;"><i class="fas fa-user-circle" style="color: #3b82f6;"></i> <?php echo htmlspecialchars($profile_user['nom']); ?></h1>
            <p class="user-type"><i class="fas fa-id-badge"></i> <?php echo htmlspecialchars(ucfirst($profile_user['type_user'])); ?></p>
            <?php if (!empty($profile_user['localisation'])): ?>
                <p class="location"><i class="fas fa-map-marker-alt"></i> <?php echo htmlspecialchars($profile_user['localisation']); ?></p>
            <?php endif; ?>
            ...
            <div class="profile-actions">
                <?php if ($is_current_user_profile): ?>
                    <a href="<?php echo BASE_URL; ?>settings.php" class="button"><i class="fas fa-edit"></i> Modifier le profil</a>
                    <?php if ($profile_user['type_user'] === 'entreprise'): ?>
                        <a href="<?php echo BASE_URL; ?>entreprise/dashboard.php" class="button primary"><i class="fas fa-briefcase"></i> Gérer l'entreprise</a>
                    <?php endif; ?>
                <?php else: ?>
                    <a href="<?php echo BASE_URL; ?>chat/?user_id=<?php echo $profile_user['id']; ?>" class="button"><i class="fas fa-comment-dots"></i> Message</a>
                    <?php if ($profile_user['type_user'] === 'entreprise'): ?>
                        <button class="button <?php echo $is_favorited ? 'secondary' : 'primary'; ?> favorite-btn" data-company-id="<?php echo $profile_user['id']; ?>">
                            <i class="fas fa-bookmark"></i> <?php echo $is_favorited ? 'Favoris' : 'Ajouter aux favoris'; ?>
                        </button>
                    <?php endif; ?>
                    <button class="button secondary report-btn" data-user-id="<?php echo $profile_user['id']; ?>"><i class="fas fa-flag"></i> Signaler</button>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="profile-content-section">
        <div class="profile-left-sidebar" style="box-shadow: 0 4px 24px rgba(0,0,0,0.07);">
            <h3 style="display: flex; align-items: center; gap: 8px;"><i class="fas fa-info-circle" style="color: #3b82f6;"></i> À Propos</h3>
            <hr style="margin: 0.5em 0 1em 0; border: none; border-top: 1px solid #e5e7eb;">
            <?php if ($profile_user['type_user'] === 'entreprise'): ?>
                <p><i class="fas fa-industry"></i> <strong>Secteur:</strong> <?php echo htmlspecialchars($profile_user['secteur'] ?? 'Non spécifié'); ?></p>
                <p><i class="fas fa-align-left"></i> <strong>Description:</strong> <?php echo nl2br(htmlspecialchars($profile_user['description'] ?? 'Pas de description.')); ?></p>
                <?php if (!empty($profile_user['horaires_ouverture'])): ?>
                    <p><i class="fas fa-clock"></i> <strong>Horaires:</strong> <?php echo htmlspecialchars($profile_user['horaires_ouverture']); ?></p>
                <?php endif; ?>
                <?php if (!empty($profile_user['telephone'])): ?>
                    <p><i class="fas fa-phone"></i> <strong>Téléphone:</strong> <?php echo htmlspecialchars($profile_user['telephone']); ?></p>
                <?php endif; ?>
                <?php if (!empty($profile_user['site_web'])): ?>
                    <p><i class="fas fa-globe"></i> <strong>Site Web:</strong> <a href="<?php echo htmlspecialchars($profile_user['site_web']); ?>" target="_blank">Visiter</a></p>
                <?php endif; ?>
                <?php if (!empty($profile_user['localisation'])): ?>
                    <p><i class="fas fa-map-marker-alt"></i> <strong>Adresse:</strong> <?php echo htmlspecialchars($profile_user['localisation']); ?></p>
                <?php endif; ?>
                ...
                <?php endif; ?>
            <?php else: ?>
                <p><i class="fas fa-user"></i> Type de compte: <?php echo htmlspecialchars(ucfirst($profile_user['type_user'])); ?></p>
                <p><i class="fas fa-map-marker-alt"></i> Localisation: <?php echo htmlspecialchars($profile_user['localisation'] ?? 'Non spécifié'); ?></p>
                <p><i class="fas fa-calendar-alt"></i> Membre depuis: <?php echo date('d/m/Y', strtotime($profile_user['date_inscription'])); ?></p>
            <?php endif; ?>
        </div>

        <div class="profile-main-content">
            <h2 style="font-size:1.3rem; color:#3b82f6; display:flex; align-items:center; gap:8px; margin-bottom:1.2em;"><i class="fas fa-stream"></i> Profil public</h2>
            <?php if ($profile_user['type_user'] === 'entreprise'): ?>
                    ...
                    <?php if ($is_current_user_profile): ?>
                        <div class="create-post-card">
                            <div class="post-header">
                                <img src="<?php echo getUserProfilePhoto($_SESSION['user_photo_profil'] ?? null); ?>" alt="Profil" class="profile-pic">
                                <div class="post-info">
                            ...
                            <form action="<?php echo BASE_URL; ?>entreprise/add_publication.php" method="GET">
                                <textarea class="post-input" placeholder="Écrivez quelque chose... (cliquez pour ajouter une annonce complète)" disabled></textarea>
                                <div class="post-actions-create">
                                    <button type="submit" class="button"><i class="fas fa-plus-circle"></i> Ajouter une annonce</button>
                                </div>
                            </form>
                        </div>
                    <?php endif; ?>
                        ...
                        ?>
                            <div class="post-card" data-post-id="<?php echo $pub['id']; ?>">
                                <div class="post-header">
                                    ...
                                    <div class="post-info">
                                        ...
                                        <?php if (!empty($pub['localisation_pub'])): ?>
                                            <span class="location"><i class="fas fa-map-marker-alt"></i> <?php echo htmlspecialchars($pub['localisation_pub']); ?></span>
                                        <?php endif; ?>
                                        <?php if (!empty($pub['categorie'])): ?>
                                            <span class="category"><i class="fas fa-tag"></i> <?php echo htmlspecialchars($pub['categorie']); ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <?php if ($is_current_user_profile): ?>
                                    <div class="post-options">
                                        ...
                                        <div class="options-dropdown">
                                            <a href="<?php echo BASE_URL; ?>entreprise/edit_publication.php?id=<?php echo $pub['id']; ?>"><i class="fas fa-edit"></i> Modifier</a>
                                            <a href="<?php echo BASE_URL; ?>entreprise/delete_publication.php?id=<?php echo $pub['id']; ?>" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette publication ?');"><i class="fas fa-trash-alt"></i> Supprimer</a>
                                        </div>
                                    </div>
                                    <?php endif; ?>
                                </div>
                                <div class="post-body">
                                    ...
                                    <?php if ($pub['image']): ?>
                                        <div class="post-media">
                                            <img src="<?php echo BASE_URL . 'uploads/' . htmlspecialchars($pub['image']); ?>" alt="Image de publication">
                                        </div>
                                    ...
                                    <?php if ($pub['document_url']): ?>
                                        <div class="post-media">
                                            <a href="<?php echo BASE_URL . 'uploads/' . htmlspecialchars($pub['document_url']); ?>" target="_blank" class="document-link">
                                ...
                                <div class="post-actions">
                                    <button class="action-btn like-btn <?php echo $is_liked ? 'liked' : ''; ?>" data-post-id="<?php echo $pub['id']; ?>">
                                ...
                                <div class="comments-section" style="display: none;">
                                        ...
                                        <?php if (!empty($comments)): ?>
                                            <?php foreach ($comments as $comment): ?>
                                                <div class="comment-item">
                                                    <img src="<?php echo getUserProfilePhoto($comment['user_photo_profil']); ?>" alt="Profil" class="comment-profile-pic">
                                                    <div class="comment-content">
                                                        ...
                                                        <span class="comment-time"><?php echo formatRelativeTime($comment['date_commentaire']); ?></span>
                                                        <?php if ($comment['note']): ?>
                                    ...
                                    <div class="add-comment-form">
                                        <form class="comment-form" data-post-id="<?php echo $pub['id']; ?>">
                                            <img src="<?php echo getUserProfilePhoto($_SESSION['user_photo_profil'] ?? null); ?>" alt="Profil" class="comment-profile-pic">
                    ...
                    <div class="add-review-form">
                        <form class="review-form" data-company-id="<?php echo $profile_user['id']; ?>">
                            <div class="form-group">
                                ...
                                <textarea id="review_content" name="review_content" placeholder="Partagez votre expérience..." required></textarea>
                            </div>
                            <div class="form-group">
                        ...
                        <?php if (!empty($company_reviews)): ?>
                            <?php foreach ($company_reviews as $review): ?>
                                <div class="review-item">
                                    <img src="<?php echo getUserProfilePhoto($review['user_photo_profil']); ?>" alt="Profil" class="comment-profile-pic">
                                    <div class="review-content">
                                        ...
                                        <span class="review-time"><?php echo formatRelativeTime($review['date_commentaire']); ?></span>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                ...
                <div id="tab-gallery" class="profile-tab-content" style="display:none;">
                    ...
                    <p>Contenu de la galerie ici. Actuellement, cette section est statique. Les uploads de la galerie seront implémentés.</p>
